...
# Purpose:       Read Sentinel-2 MSI L1C data from SAFE into a single
...
class Sentinel2_reader_and_NetCDF_converter:
    ''' Class for reading Sentinel-2 MSI L1C products from SAFE with methods for
    ...
    def main(self):
        ...
        if not self.dterrengdata:
            self.readSunAndViewAngles(self.SAFE_path + self.xmlFiles['S2_Level-1C_Tile1_Metadata'])
        else:
    ...
    def initializer(self, mainXML):
        ...
        # Set gdal object
        self.src = gdal.Open(str(fdirName +self.xmlFiles['S2_Level-1C_Product_Metadata']))
    ...
    def write_to_NetCDF(self, nc_outpath, compression_level):
        ...
        with (netCDF4.Dataset(out_netcdf, 'w', format='NETCDF4')) as ncout:
            ...
            if not self.dterrengdata:
                    ...
                    if not "True color image" in v:
                        for i in range(1,subdataset.RasterCount+1):
                            ...
                            varout = ncout.createVariable(varName,np.uint16,
                                    ('time', 'y', 'x'), fill_value=0, zlib=True, complevel=compression_level)
                            varout.units = "1";
                        ...
                        varout = ncout.createVariable('TCI','u1',
                                ('dimension_rgb', 'y', 'x'), fill_value=0, zlib=True, complevel=compression_level)
                ...
                bands_alias = {'B01':'B1', 'B02':'B2', 'B03':'B3', 'B04':'B4',
                               ...
                               'B11':'B11', 'B12':'B12'}
                for raster_path in self.image_list:
            ...
            for layer,path in self.vectorInformation.iteritems():
                if path:
            ...
            counter = 1
            for k,v in self.sunAndViewAngles.iteritems():
                ...
                varout = ncout.createVariable(k ,np.float32,('time', 'y', 'x'), fill_value=netCDF4.default_fillvals['f4'], zlib=True)
                varout.units = 'degree'
                if 'sun' in k:
            ...
            for k,f in self.xmlFiles.iteritems():
                if f:
                    if f.endswith('.xml'):
                        if not self.dterrengdata:
            ...
            ncout.title =  'Sentinel-2 L1C data'
            ...
            self.globalAttribs['summary'] = 'Sentinel-2 Multi-Spectral Instrument Level-1C product.'
    ...
    def readSunAndViewAngles(self,xmlfile):
        ...
        nx = int(root.xpath(str('//n1:Level-1C_Tile_ID/n1:Geometric_Info/Tile_Geocoding/Size[@resolution=10]/NROWS'),namespaces=root.nsmap)[0].text) # nb of rows
        ny = int(root.xpath(str('//n1:Level-1C_Tile_ID/n1:Geometric_Info/Tile_Geocoding/Size[@resolution=10]/NCOLS'),namespaces=root.nsmap)[0].text) # nb of columns
        ...
        # View angles
        counter_angle = 0
        for BANDID in np.array(self.bands_alias_bandID.keys()):
