...
class Sentinel2_reader_and_NetCDF_converter:
    ...
    def __init__(self, SAFE_file, SAFE_outpath):
        ...
        self.bands_alias_bandID = {0:'B1', 1:'B2', 2:'B3', 3:'B4', 4:'B5',
            5:'B6', 6:'B7', 7:'B8', 8:'B8A', 9:'B9', 10:'B10', 11:'B11',
            12:'B12'}
    ...
    def main(self):
        ...
        if not self.dterrengdata:
            self.readSunAndViewAngles(self.SAFE_path + self.xmlFiles['S2_{}_Tile1_Metadata'.format(self.processing_level)])
            #self.readSunAndViewAngles(self.SAFE_path + self.xmlFiles['S2_Level-2A_Tile1_Metadata'])
        else:
        ...
        for f in list(self.xmlFiles.values()):
            if f:
                if f.endswith('.gml'):
                    if not self.dterrengdata:
                        gmlfile = self.SAFE_path + f
                    else:
                        gmlfile = f
    ...
    def initializer(self, mainXML):
        ...
        fdirName = '%s%s.SAFE' % (self.SAFE_outpath,self.SAFE_id)
        tree = ET.parse(mainXML)
        root = tree.getroot()
    ...
    def write_to_NetCDF(self, nc_outpath, compression_level, chunk_size=(1,32,32)):
        ...
        nx = self.reference_band.RasterXSize # number of pixels for 10m spatial resolution frequency bands
        ny = self.reference_band.RasterYSize # number of pixels for 10m spatial resolution frequency bands

        # output filename
        out_netcdf = '%s%s.nc' % (nc_outpath,self.SAFE_id)

        with (netCDF4.Dataset(out_netcdf, 'w', format='NETCDF4')) as ncout:
            dim_time = ncout.createDimension('time',0)
            dim_x = ncout.createDimension('x',nx)
            dim_y = ncout.createDimension('y',ny)
            ...
            # NODATA = 0 (ie. fillvalue) from https://sentinel.esa.int/documents/247904/685211/Sentinel-2-Products-Specification-Document
            ##########################################################
            # Status
                    ...
                    if not "True color image" in v:
                        for i in range(1,subdataset.RasterCount+1):
                            ...
                            varout = ncout.createVariable(varName,np.uint16,
                                    ('time', 'y', 'x'), fill_value=0, zlib=True, complevel=compression_level, chunksizes=chunk_size)
                            varout.units = "1";
                        ...
                        varout = ncout.createVariable('TCI','u1',
                                ('time','dimension_rgb', 'y', 'x'), fill_value=0, zlib=True, complevel=compression_level, chunksizes=(1,)+chunk_size)
                ...
                for raster_path in self.image_list_dterreng:
                    ...
                    if not "TCI" in raster_path:
                        for i in range(1,subdataset.RasterCount+1):
                            ...
                            varout.grid_mapping = "UTM_projection";
                            varout.standard_name = 'toa_bidirectional_reflectance'
                            varout.long_name = 'Reflectance in band %s' % bands_alias[raster_path.split('.')[-2][-3::]]
                            #varout.bandwidth = band_metadata['BANDWIDTH']
                            #varout.bandwidth_unit = band_metadata['BANDWIDTH_UNIT']
                            #varout.wavelength = band_metadata['WAVELENGTH']
                            #varout.wavelength_unit = band_metadata['WAVELENGTH_UNIT']
                ...
                if path:
                    ...
                    if rasterized_ok:
                        if layer == "MSK_CLOUDS_B00":
                            varout = ncout.createVariable('Clouds', 'i1', ('time','y','x'),fill_value=-1,zlib=True, chunksizes=chunk_size)
                            varout.long_name = "Cloud mask 10m resolution"
                            varout.comment= "Rasterized cloud information."
                        else:
                            varout = ncout.createVariable(layer, 'i1', ('time','y','x'),fill_value=-1,zlib=True, chunksizes=chunk_size)
                        ...
                        varout.flag_meanings = ' '.join([key.replace('-','_') for key in list(layer_mask.keys())])
                        vector_band = gdal.Open(output_file)
            ...
            if self.processing_level=='Level-2A':
                ...
                for k,v in list(l2a_kv.items()):
                    ...
                    DataType = gdal_nc_data_types[gdal.GetDataTypeName(SourceDS.GetRasterBand(1).DataType)]
                    ...
                    if varName == "SCL":
                        varout.flag_values = np.array(list(scene_classifcation_flags.values()),dtype=np.int8)
                        varout.flag_meanings = ' '.join([key for key in list(scene_classifcation_flags.keys())])

                    if GeoT[1] != 10:
                        raster_data = scipy.ndimage.zoom(input=SourceDS.GetVirtualMemArray(), zoom=nx/xsize, order=0)
                    else:
            ...
            for k,v in list(self.sunAndViewAngles.items()):
                ...
                varout = ncout.createVariable(k ,np.float32,('time', 'y', 'x'), fill_value=netCDF4.default_fillvals['f4'], zlib=True, chunksizes=chunk_size)
                varout.units = 'degree'
                if 'sun' in k:
            ...
            # Add xml files as character values see: https://stackoverflow.com/questions/37079883/string-handling-in-python-netcdf4
            ##########################################################
            # Status
            ...
            self.globalAttribs['summary'] = 'Sentinel-2 Multi-Spectral Instrument {} product.'.format(self.processing_level)
    ...
    def readSunAndViewAngles(self,xmlfile):
        ...
        nx = int(root.xpath(str('//n1:{}_Tile_ID/n1:Geometric_Info/Tile_Geocoding/Size[@resolution=10]/NROWS'.format(self.processing_level)),namespaces=root.nsmap)[0].text) # nb of rows
        ny = int(root.xpath(str('//n1:{}_Tile_ID/n1:Geometric_Info/Tile_Geocoding/Size[@resolution=10]/NCOLS'.format(self.processing_level)),namespaces=root.nsmap)[0].text) # nb of columns
            ...
            for incidence_angles in incidence_angles_list:
                if int(incidence_angles.attrib['bandId']) == BANDID:
                    ...
                    for angle in incidence_angles:
                        ...
                        for value_entry in values_list[0:-1]:
                            if angle.tag == 'Zenith':
                                tmp_angle = np.array([float(i) for i in value_entry.text.split()])[0:-1]
                                tmp_view_zenith[counter_entry,np.isnan(tmp_angle)==False] = tmp_angle[np.isnan(tmp_angle)==False]
                                counter_entry += 1
                            if angle.tag == 'Azimuth':
                                tmp_angle = np.array([float(i) for i in value_entry.text.split()])[0:-1]
                                tmp_view_azimuth[counter_entry,np.isnan(tmp_angle)==False] = tmp_angle[np.isnan(tmp_angle)==False]
                                counter_entry += 1
                    counter_angle += 1
                self.sunAndViewAngles[str('view_zenith_' + self.bands_alias_bandID[BANDID])] = tmp_view_zenith
                self.sunAndViewAngles[str('view_azimuth_' + self.bands_alias_bandID[BANDID])] = tmp_view_azimuth
        ...
        for i in range(angles_length):
            for j in range(angles_height):
                if not i == angles_length-1 and not j == angles_height-1:
                    angles_resampled[i*step:i*step+step,j*step:j*step+step] = angles[i,j]
                else:
    ...
    def readVectorInformation(self,gmlfile):
        ...
        output_dir = str(self.SAFE_path + '/tmp/')
        ...
        vt = gdal.VectorTranslate(destNameOrDestDS=destName,srcDS=gmlfile,options=vto)
        vt.FlushCache()
        vt = None

        if os.path.isfile(destName) == True:
            return gmlID, destName
        else:
            return gmlID, False

    def rasterizeVectorLayers(self, nx, ny, layer_name, shapefile, output_file): #,output_dir=str(self.SAFE_path + '/tmp/'), maskType=None):
        """ Supports all vector layers. Rasterized by means of gdal tools."""
        ulx, xres, xskew, uly, yskew, yres  = self.reference_band.GetGeoTransform() #ulx - upper left x, uly - upper left y
        ...
        if shapefile:
            if maskType:
                ...
                ro =  gdal.RasterizeOptions(xRes=xres, yRes=yres,
                    ...
                    layers=[layer_name], noData=0)
            tmp = gdal.Rasterize(destNameOrDestDS=str(output_dir + layer_name + '.tif'),srcDS=shapefile, options=ro)
            tmp.FlushCache()
            tmp=None
        vector_fn = src
        nx = ny = 10980
        ...
        target_ds = gdal.GetDriverByName('GTiff').Create(output_file, nx, ny, 1, gdal.GDT_Byte)
    ...
    def genLatLon(self,nx,ny,latlon=True):
        ...
        xp = np.int32(ulx+(xres*0.5)) + indices[1]*np.int32(xres) + indices[1]*np.int32(xskew)
        yp = np.int32(uly-(yres*0.5)) + indices[0]*np.int32(yres) + indices[0]*np.int32(yskew)
...
def main():
    SAFE_file = '/home/trygveh/data/satellite_data/Sentinel-2/l1c/S2B_MSIL1C_20200614T104629_N0209_R051_T32VNL_20200614T132040.zip'
    SAFE_outpath = '/home/trygveh/tmp/'
    nc_outpath = '/home/trygveh/tmp/'
    cl = 7
    conversion_object = Sentinel2_reader_and_NetCDF_converter(SAFE_file=SAFE_file, SAFE_outpath=SAFE_outpath)
